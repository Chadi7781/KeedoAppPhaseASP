...
namespace KeedoApp.Controllers
{
    public class MessageController : Controller
    {
        ...
        public async Task<ActionResult> Send(int id, Message message)
        {
            ...
            {
                return RedirectToAction("Index2", new { id = id, name = user.ReturnFirstLast });
            }
            return View(message);
        }

        public ActionResult Connect(int id, String name)
        {
            HttpClient client = new HttpClient();
            client.BaseAddress = new Uri("http://localhost:8080/SpringMVC/servlet/");

            client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
            HttpResponseMessage httpResponseMessage = client.GetAsync("message/connect").Result;
            return RedirectToAction("Index2", new { id = id, name = name });
        }
    }
}